...
| [#5 — M5: Metrics-watcher](https://github.com/mikelord007/Chaos_Notary/pull/5) | 3 bugs (all High, Correctness): `observedSeverity`/`verdict` were derived from error rate alone, so a latency-only fault below checkout-api's 2000ms timeout got a false "milder than predicted" verdict despite the tool having measured the real latency degradation; the query-window buffer added in an earlier fix round had a genuine mathematical flaw — a fixed +60s buffer became a larger fraction of the window than a short fault itself, diluting the observed error rate below the "hard" threshold; a full `chaos-checkout-api` outage was misreported as "none" because the API's own request counters can't increment while the process is frozen, and the tool never checked Prometheus's independent `up{}` scrape-health signal | All 3 fixed in [90e5f6b](https://github.com/mikelord007/Chaos_Notary/commit/90e5f6b)/[14a8b4f](https://github.com/mikelord007/Chaos_Notary/commit/14a8b4f) — a real design fix (PromQL time-anchoring via the fault tool's own `expiresAt` timestamp, plus an `up{}`-based outage check), not another heuristic; follow-up Qodo review confirmed all 3 fixes correctly reflected in the diff, including the exact scenarios flagged. This PR also got this project's first-ever live Docker verification, which surfaced a 4th Critical bug beyond Qodo's findings (a shared MCP transport that broke every call after the first — see [#6](https://github.com/mikelord007/Chaos_Notary/pull/6)'s note below); fixed in [a5f4eb7](https://github.com/mikelord007/Chaos_Notary/commit/a5f4eb7), confirmed by Qodo and by `bash scripts/verify-m5.sh` passing end to end against a real Docker daemon |
| [#6 — fix: live-verification stack bugs](https://github.com/mikelord007/Chaos_Notary/pull/6) | Not a Qodo-reviewed PR — merged directly on the strength of live verification alone (explicit, deliberate exception to this project's usual Qodo-review-per-PR convention). Docker finally worked in this project's environment for the first time this session; bringing the stack up live immediately found 4 real bugs that had survived every prior review and merge: `services/mcp-server/Dockerfile` pinned a Pumba image tag that doesn't exist on Docker Hub (stack was never buildable), `checkout-api`'s healthcheck probed the wrong loopback address and silently blocked `mcp-server`/`loadgen` from ever starting, `scripts/verify-m1.sh`'s replication check never passed a password to `psql`, and `mcp-server`/`blast-radius-sandbox` shared one MCP transport instance across their entire process lifetime — the first request ever received succeeded, every request after that failed with a bare 500, invisible to every unit test since they all use an in-process transport pair | All 4 fixed and verified live: `bash scripts/verify-m1.sh`, `verify-m2.sh`, and `verify-m4.sh` all now pass end to end against a real Docker daemon for the first time in this project's history |
